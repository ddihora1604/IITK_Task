...
output_dir = os.path.join("Datasets", "Income Statement")
...
for i, ticker_name in enumerate(tickers):
    try:
        ...
        # Export to CSV
        output_file = os.path.join(output_dir, f"{ticker_name}_income_statement.csv")
        income_stmt.to_csv(output_file)
...
print(f"Successfully fetched Income Statement data for {success_count} Tickers")
if error_count > 0:
    print(f"Failed to fetch data for {error_count} tickers: {', '.join(error_tickers)}")
print(f"Data collection complete, files saved to 'Datasets/Income Statement/' folder.")