...
output_dir = os.path.join("Datasets", "Balance Sheet")
...
for i, ticker_name in enumerate(tickers):
    try:
        ...
        # Export to CSV
        output_file = os.path.join(output_dir, f"{ticker_name}_balance_sheet.csv")
        balance_sheet.to_csv(output_file)
...
print(f"Successfully fetched Balance Sheet data for {success_count} Tickers")
if error_count > 0:
    print(f"Failed to fetch data for {error_count} tickers: {', '.join(error_tickers)}")
print(f"Data collection complete, files saved to 'Datasets/Balance Sheet/' folder.")